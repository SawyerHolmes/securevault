...
const expandedPwd         = document.getElementById("expanded-password");
...
const copyPasswordFullBtn = document.getElementById("copy-password-full-btn");
...
editBtn.addEventListener("click", () => {
    ...
    expandedPwd.innerHTML      = `<input type="${passwordVisible ? "text" : "password"}" id="edit-password" class="value" value="${e.password || ""}">`;
...
saveBtn.addEventListener("click", () => {
    ...
    saveVault();
    renderVault(searchInput.value); // update grid card immediately
    exitEditMode();
});
...
function exitEditMode() {
    ...
    expandedPwd.textContent      = passwordVisible ? e.password : "••••••••";
...
togglePasswordBtn.addEventListener("click", () => {
    ...
    if (editInput) {
        ...
        expandedPwd.textContent = passwordVisible ? vault[currentIndex].password : "••••••••";
    }
...
copyIconBtn.addEventListener("click", () => {
    if (currentIndex === null) return;
    copyText(vault[currentIndex].password, "Password copied!");
});

copyPasswordFullBtn.addEventListener("click", () => {
    if (currentIndex === null) return;
    copyText(vault[currentIndex].password, "Password copied!");
});
...
copyUsernameBtn.addEventListener("click", () => {
    if (currentIndex === null) return;
    copyText(vault[currentIndex].username, "Username copied!");
});
...
// Close sort menu when search focused
...
renderVault();const expandedPassword  = document.getElementById("expanded-password");
const expandedNotes     = document.getElementById("expanded-notes");
const editBtn           = document.getElementById("edit-btn");
const saveCancelWrapper = document.getElementById("save-cancel-wrapper");
const saveBtn           = document.getElementById("save-btn");
...
function renderVault(filter = "") {
    ...
    if (!displayVault.length) {
        ...
        emptyDiv.innerHTML = `
            <div class="empty-icon"><i class="fa-solid fa-vault"></i></div>
            <h2>${q ? "No results" : "Your vault is empty"}</h2>
            <p>${q ? "Try a different search term" : "Add your first entry to get started"}</p>
            ${!q ? `<a href="add-entry.html" class="btn-black empty-add-btn">Add Entry</a>` : ""}
        `;
...
function enterEditMode() {
    ...
    const e = vault[currentIndex];
    document.getElementById("url-field").style.display = "";
    expandedUrl.innerHTML      = `<input type="text" id="edit-url" class="value expanded-value" value="${e.url || ""}">`;
    expandedUsername.innerHTML = `<input type="text" id="edit-username" class="value expanded-value" value="${e.username || ""}">`;
    expandedPassword.innerHTML = `<input type="${passwordVisible ? "text" : "password"}" id="edit-password" class="value expanded-value" value="${e.password || ""}">`;
    expandedNotes.innerHTML    = `<textarea id="edit-notes" class="value expanded-value">${e.notes || ""}</textarea>`;
}
...
saveBtn.addEventListener("click", () => {
    ...
    saveVault();
    exitEditMode();
    renderVault(searchInput.value); // re-renders grid card in place
});
...
// =========================
// SEARCH — works even while sort menu is open
// =========================
searchInput.addEventListener("input", () => renderVault(searchInput.value));

// Close sort menu when search is focused
searchInput.addEventListener("focus", () => {
    sortMenu.style.display = "none";
    sortToggle.classList.remove("active");
});
